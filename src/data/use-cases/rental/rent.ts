import { GetBooksByIdsRepository } from "src/data/protocols/db/book/findBooksByIdsRepository";
...
export class Rent implements IRent{
    constructor(
        private findBooksByIdsRepository : GetBooksByIdsRepository,
        private findReaderByIdRepository : FindReaderByIdRepository
    ){}
    async execute(idBooks: string[], readerId: any): Promise<any>{
        const books = await this.findBooksByIdsRepository.getBooksByIdsRepository(idBooks)
        if(!books){
            return null
        }
